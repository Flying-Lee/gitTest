...
-(CGFloat)setMyPhotoArray:(NSMutableArray*)photoArray{
   ...
   _photoAddViewHigh = 0;
    ...
    if ([self.delegate respondsToSelector:@selector(photo:)]) {
        
        [self.delegate photo:_photoAddViewHigh];
    }
...
-(void)photoAlert{
    ...
    NSString *title = [NSString stringWithFormat:@"还可添加%zd张图片", _maxPhotoCount - self.photoArray.count + 1];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:title message:@"将访问您的相机或者相册" preferredStyle:UIAlertControllerStyleActionSheet];
...
-(void) openPhotos{
    ...
    pickerVc.maxCount = _maxPhotoCount;
    ...
    pickerVc.callBack = ^(NSArray *assets) {
        if(assets.count+_photoArray.count > _maxPhotoCount+1){
            ...
            [assets enumerateObjectsUsingBlock:^(ZLPhotoAssets *  _Nonnull zl, NSUInteger idx, BOOL * _Nonnull stop) {
                [_photoArray addObject:zl.originImage];
                
                NSString *str = [_formatter stringFromDate:[NSDate date]];
                NSString *fileName = [NSString stringWithFormat:@"%@%zd.png", str,idx];
                [_photoPathArray addObject:fileName];
            }];
